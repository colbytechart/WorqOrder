...
|---|---|---|---|---|
| Logical versioned two-entry ZIP | versioned DTO/strict JSON codec/manifest foundation in M50; bounded writer in M51; bounded strict reader in M52 | DTO/strict-shape plus production exact-entry/Deflate/byte-count/SHA-256/bounds/cancellation/read-rejection tests pass | inspect/share/import created file after Settings integration | Partial: model, writer, reader, and non-UI import engine pass; Settings picker remains |
| Complete portable state and exclusions | logical DTO/exclusion allowlist in M50; Room snapshot and portable preference adapters in M51; exact replacement/equivalence in M52 | complete/empty/Unicode fixtures, forbidden-field audit, Room-backed snapshot/replacement, and exact logical round-trip tests pass | populated cross-device comparison after Settings integration | Partial: non-UI capture/replacement and exclusion evidence pass; end-to-end UI/device comparison remains |
| Atomic import and crash recovery | M52A frozen protocol plus M52B-D application lock, verified restore point, self-checking next-action journal, transactional Room and exact Preferences replacement, and startup convergence | Room postconditions, preference equivalence, every-phase fault/restart/idempotency/cancellation/rollback tests and full JVM/connected suites pass by owner report | force-stop/reboot during Settings-integrated safe scenarios | Partial: engine and automated crash-integrity gates pass; user-visible lifecycle gate remains |
| Swap-style one-generation Restore | M52 verified no-backup point, displaced-state capture, journaled swap/finalization, and exact rollback authority | swap directions, repeated reconciliation, corruption/failure preservation, and exact DTO equivalence pass | import, restore, swap-back, and uninstall behavior after Settings integration | Partial: engine and automated swap evidence pass; user-visible device gate remains |
...
| Settings UX/accessibility | `BackupRestoreViewModel`, compact Settings card, SAF launchers, typed persistent status | state, semantics, confirmation, duplicate-submit, picker-cancel, timer-race JVM/Compose coverage, final compilation/JVM/lint/build gate, and focused connected Settings test pass by owner report | section placement, timer-disabled state, rotation, large text/display, picker cancellation, and compact status-first presentation pass by owner report | Complete for Milestone 53; fixture-backed end-to-end Import/Restore hardening remains assigned to Milestone 54 |
