...
| 14-column canonical schema 3 with repeated Start/End dates | Shared export row builder/snapshot | exact headers, `MM/DD/YYYY` duplicated dates, CSV/XLSX/Google equivalence, schema-2 owned-tab upgrade passed | Open representative files/sheet | Pass automated; representative populated UI/export release gate remains M29 |
| Client-name CSV import append/A–Z | `ClientCsvParser`, Android bounded document source, import coordinator, Client DAO transaction, Client Management UI | UTF-8/BOM/quoting/line breaks/limits, file policy, normalization/duplicates, restore/rollback, ViewModel and Compose states | Picker/import sample remains a release manual gate | Partial: automated M20 coverage implemented; manual picker sample pending |
| Consultant directory/selection/history | Employee Room repository, `ConsultantSelectionCoordinator`, typed DataStore selection, Consultant Settings/Create/Edit UI, transaction-captured task snapshot | repository/coordinator/ViewModel plus Room snapshot, active-reference race, persistence, and connected UI tests passed | Consultant Settings and Create/Edit assignment workflow | Pass through M22; 93-test connected gate passed |
| Work Type and Mileage | Task metadata/repository/Create/Edit UI | persistence, migration, canonical decimal/input validation, ViewModel, and Compose coverage passed | Create/edit workflow | Pass through M22; 93-test connected gate passed |
