...
| Consultant directory/selection/history | Employee Room repository, `ConsultantSelectionCoordinator`, typed DataStore selection, Consultant Settings UI, transaction-captured task snapshot | repository/coordinator/ViewModel plus Room snapshot and persistence tests | Consultant Settings add/rename/archive/restore/select and Create Task missing-selection route | Pass for M21 directory/selection/snapshot scope; task metadata correction remains M22 |
